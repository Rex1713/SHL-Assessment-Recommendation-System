...
async def recommend_assessments(query: str = Query(..., description="Text query or URL")):
    # If it's a URL, extract content from it
    ...
    for node in response.source_nodes:
        ...
        duration = (
            "Untimed" if meta["duration_minutes"] == 9999
            else "Variable" if meta["duration_minutes"] == -1
            else f"{meta['duration_minutes']} mins"
        )
    ...
    # Format summary as bullet list
    lines = response.response.strip().split("\n")
    formatted_summary = "\n".join(
        f"- {line.strip()[3:]}" if line.strip().startswith(tuple(f"{i}." for i in range(1, 21))) else line.strip()
